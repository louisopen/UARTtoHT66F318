...
void Uart_RXD_Manage(void)
{
	if(uart_rx_success == 1)	//Is't finished ?
	{
		...
		if(array_uart_rxbuff[0] == 0x44 && array_uart_rxbuff[RX_DATA_MAX-1]==0x0A)
		{
			...
			{
				case 0x01:	//request: 44 01 00 00 00 08 A0 0A

					
					break;
				case 0x02:	//request: 44 02 00 00 00 08 A0 0A
					Uart_TXD_Send(0x55AA);
					
					break;
				case 0x03:	//request: 44 03 00 00 00 08 A0 0A


					break;				
				case 0x06:	//request: 44 06 00 00 00 08 A0 0A
...
void Uart_TXD_Send(u16 tx_data)
{
	...
	array_uart_txbuff[1]	= 0x02;		//function or command
